...
class phpspider
{
    ...
    /**
     * 设置代理 回调 libaoan 2016/12/23
     * 
     * @var mixed
     * @access public
     */
     public $on_set_proxy = null;
     
     /**
     * 入库操作 回调  libaoan 2016/12/24 
     */
     public $on_insert_db = null;
    ...
    public function parse_command()
    {
        ...
        case 'start':
            ...
            //如果进程已存在 则退出 libaoan 2016/12/24
            exec("ps aux | grep $start_file | grep -v grep", $info);
            if(count($info)>=2) {
                exit("PHPSpider[$start_file] process exist Ready to quit \n");            
            }
            if ($command2 === '-d') 
            {
                self::$daemonize = true;
    ...
    public function collect_page() 
    {
        ...
        if ($this->is_anti_spider) 
        {
            ...
            if ($is_anti_spider) 
            {
                //反爬虫后切换新的代理ip, 需自己实现
                //libaoan 2016/12/23
                if($this->on_set_proxy) {
                    $new_proxy = call_user_func($this->on_set_proxy, $this);
                    if(!empty($new_proxy)) {
                        self::$configs['proxy'] = $new_proxy;
                    }
                }
        ...
        if (!empty($fields)) 
        {
            ...
            if (isset($fields) && is_array($fields)) 
            {
                ...
                if (!empty(self::$configs['export'])) 
                {
                    ...
                    {
                        //入库操作回调  libaoan 2016/12/24
                        if($this->on_insert_db) {
                            call_user_func($this->on_insert_db, $fields, $page, $this);    
                        } else {
